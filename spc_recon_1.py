...
def dmd_random_patterns(width: int, height: int,
    ...
    A single call of that form needs ~15 GB of float64 working memory at
    DMD resolution.  We split it into batches of ``batch`` patterns -- the
    RNG state advances identically, so the concatenated output is bit-
    ...
    """
    np.random.seed(seed)
    out = np.empty((num_patterns, height, width), dtype=dtype)
    for i in range(0, num_patterns, batch):
        b = min(batch, num_patterns - i)
        chunk = np.random.choice(
            [0, 255], size=(b, height, width, 1),
            p=[1 - on_probability, on_probability]).astype(dtype)
        out[i:i + b] = chunk[..., 0]
    return out
